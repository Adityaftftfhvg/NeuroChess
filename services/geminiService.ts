...
export const geminiService = {
   ...
   */
sendChatMessage: async (history, fen, userMessage): Promise<string> => {
  try {
    const contextMessage = `[Current Board FEN: ${fen}] User says: ${userMessage}`;
    const contents = [
      ...history.map(h => ({ role: h.role === 'model' ? 'assistant' : h.role, parts: h.parts })),
      { role: 'user', parts: [{ text: contextMessage }] }
    ];
    const response = await ai.models.generateContent({
      model: 'gemini-2.0-flash',
      contents,
      config: { systemInstruction: SOCRATIC_SYSTEM_INSTRUCTION }
    });
